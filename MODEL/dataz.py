...
# midi_folder = '/home/dxwoo/Documents/Code_2/Perceiver-Music-Transformer/MIDIDBNANO'
# output_folder = '/home/dxwoo/Documents/Code_2/Perceiver-Music-Transformer/PICKEL'

current_directory = os.getcwd()

midi_folder = os.path.join(current_directory, 'MIDIDBNANO')
output_folder = os.path.join(current_directory, 'PICKEL')
...
TMIDIX.Tegridy_Any_Pickle_File_Writer(filez, os.path.join(output_folder, 'filez'))

filez = TMIDIX.Tegridy_Any_Pickle_File_Reader(os.path.join(output_folder, 'filez'))
              ...
              if avg_time < 96 and instruments_list != [9]: # Filtering out bad MIDIs...
                if 0 in times_list: # Filtering out (mono) melodies MIDIs
                  if len(melody_chords) > 10:            
                    ...
                    num_batches = (len(mel_cho) // 75) - 3
                    
                    for i in range(num_batches):
                        
                        mel_cho1 = []
                        mel_cho1.extend(mel_cho[i*25*4:((i+1)*25*4)])
                        mel_cho1.extend(mel_cho[(i+1)*25*4:((i+2)*25*4)])
                        mel_cho1.extend(mel_cho[(i+2)*25*4:((i+3)*25*4)])
                    ...
                    if files_count % 5000 == 0:
                      ...
                      TMIDIX.Tegridy_Any_Pickle_File_Writer(melody_chords_f, output_folder+count)
                      melody_chords_f = []
                      print('=' * 70)
